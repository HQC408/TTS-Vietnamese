...
def text2tokens(text, lexicon_fn):
    ...
    for word in words:
        if word in FLAGS.special_phonemes:
            ...
            for pp in p:
                if pp in phonemes:
                    tokens.append(phonemes.index(pp))
                else:
                    print(f"Warning: Phoneme '{pp}' not found in phoneme list.")
                    tokens.append(FLAGS.sil_index)  # Thay thế bằng silence index
            ...
            for p in word:
                if p in phonemes:
                    ...
                    print(f"Warning: Phoneme '{p}' not found in phoneme list.")
                    tokens.append(FLAGS.sil_index)  # Thay thế bằng silence index
            tokens.append(FLAGS.word_end_index)
